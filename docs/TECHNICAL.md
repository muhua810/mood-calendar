...
| AI 情绪分析 | 识别五级情绪类型和强度 | Workers AI 代理 + 本地 140+ 关键词 + 29 否定词 + Emoji + 分句加权 + 反讽检测 + 网络用语 + OpenAI API 三级降级 |
| 情绪热力图日历 | GitHub 贡献图风格，支持年份导航 | 自研 SVG 组件 |
| 情绪趋势图表 | 饼图 + 面积图 + 柱状图 | Recharts |
| 年度报告 | 年度封面、核心数据、最佳月份、情绪图谱、年度寄语 | 统计分析 Tab |
...
│  │          共享服务层                   │ │
│  │  ┌──────────┐  ┌────────────────────┐ │ │
│  │  │ Storage  │  │ EmotionAnalyzer    │ │ │
│  │  │ Service  │  │ (140+ 关键词       │ │ │
│  │  └────┬─────┘  │  + Emoji + 分句    │ │ │
│  │  ┌────┴─────┐  │  + 反讽 + 网络用语)│ │ │
│  │  │ Reminder │  └────┬───────────────┘ │ │
│  │  │ Service  │  │ demoData │ apiService│ │
│  │  │          │  │ backupService        │ │
│  │  └──────────┘  └─────────────────────┘ │ │
│  └───────┼──────────────────────────────┘ │
│          │                                │
│   ┌──────▼──────┐  ┌──────────────────┐  │
│   │ LocalStorage│  │ Workers AI 代理  │  │
│   └─────────────┘  │ / OpenAI API     │  │
│                    └──────────────────┘  │
│                                          │
│  ┌──────────────────────────────────────┐ │
│  │  PWA: Service Worker (SWR 缓存)      │ │
│  └──────────────────────────────────────┘ │
└──────────────────────────────────────────┘
```
...
输入文本
  │
  ├──→ Workers AI 代理（默认，DeepSeek）
  │       通过 Cloudflare Workers 转发，API Key 安全存储在环境变量
  │       返回结构化 JSON（mood/intensity/analysis/keywords/suggestion）
  │       失败时降级
  │
  ├──→ 用户自定义 API（可选，OpenAI 兼容）
  │       用户配置 API Key（AES-256-GCM 加密存储）
  │       返回结果经过字段校验（mood 枚举 + intensity 范围）
  │       失败时降级
  │
  └──→ 本地关键词匹配（兜底，即时）
          分句加权：按标点分割句子，对每个分句独立分析
          反转模式："虽然...但是..."后半句权重 2.5x
          Emoji 映射：50+ emoji 作为额外情绪信号
          关键词库：五级 140+ 关键词
          否定词检测：29 个否定词，10 字符窗口
          相对化表达弱化
```
