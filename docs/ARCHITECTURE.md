...
心迹（MoodTrace）是一款面向大学生群体的情绪追踪与心理健康可视化 Web 应用。用户每天用一句话记录心情，系统自动分析情绪类型并生成热力图日历，帮助用户建立情绪觉察习惯。

### 1.1 设计理念

- **低门槛**：一句话记录，不需要长篇大论
- **隐私优先**：纯前端核心数据 + 可选伪匿名统计，用户掌控一切
- **智能分析**：Workers AI 代理 + 本地关键词 + Emoji + 分句加权 + 可选云端 AI，多层降级策略
...
| 技术 | 版本 | 选型理由 |
|------|------|----------|
| React | 19 | 最新稳定版本，性能优化、新 Hooks API |
...
| Cloudflare Workers | - | 边缘计算后端，低延迟伪匿名统计 API |
...
| i18n | 自研 | 轻量国际化模块，零依赖，支持 6 种语言 |

### 2.1 为什么选择"前端为主 + 轻量后端"架构

| 考量 | 方案 | 理由 |
|------|------|------|
| 隐私 | 前端存储核心数据 | 情绪数据敏感，仅伪匿名统计上云 |
...
| `moodtrace_lang` | 语言偏好（i18n，支持 zh/en/ja/ko/fr/es） |
...
轻量级自研 i18n 方案，零第三方依赖：

- **`translations.js`** — 翻译字典，支持 zh（中文，默认）、en、ja、ko、fr、es 共 6 种语言
- **`index.js`** — 工具模块，提供 `t(key)` 翻译函数、`getCurrentLang()` / `setCurrentLang()` 语言读写、`getSupportedLangs()` 语言列表
- **存储**：语言偏好通过 `localStorage` 持久化（key: `moodtrace_lang`），默认 `zh`
- **切换机制**：设置页 3×2 网格按钮切换语言 → `setCurrentLang()` → `window.location.reload()` 全局刷新生效
- **降级策略**：找不到翻译时回退到中文，中文也找不到时返回 key 本身
