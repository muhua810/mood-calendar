...
```
getCareLevel(consecutiveDays, currentMood)
    │
    ├─ currentMood === 'very_negative'
    │   ├─ consecutiveDays >= 3 → Level 3（重度）
    │   ├─ consecutiveDays >= 1 → Level 2（中度）
    │   └─ return Level 1（轻度）
    │
    ├─ currentMood === 'negative'
    │   ├─ consecutiveDays >= 5 → Level 3（重度）
    │   ├─ consecutiveDays >= 3 → Level 2（中度）
    │   └─ consecutiveDays >= 2 → Level 1（轻度）
    │
    └─ return 0（不显示）
```
...
输入文本 → toLowerCase()
    │
    ├─ 检查危机关键词（想死、不想活等）→ 直接触发 Level 3 关怀
    │
    ▼
反讽检测 (detectSarcasm, 6种模式)
    │
    ▼
