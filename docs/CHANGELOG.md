## v2.1.3 (2026-03-29) — 情绪洞察引擎 + 代码重构 + 安全加固

### 🧠 新功能：情绪洞察引擎 (InsightEngine)
- **主动关怀模块**：基于历史记录自动分析 6 种模式，首页主动展示洞察
  - 情绪连续下降趋势检测（最近 3-5 天持续走低）
  - 压力关键词频率异常（考试、面试、分手等 20+ 关键词）
  - 记录间隔突然增大检测（可能表示逃避/低落）
  - 周期性模式识别（如周一总是低落）
  - 积极改善检测（最近情绪明显好转）
  - 里程碑庆祝（连续记录 7/14/30/60/100 天）
- 按严重程度排序，最多展示 2 条（避免信息过载）
- 支持单条关闭，当日不再提醒
- 新增 `src/services/insightEngine.js` + `src/components/InsightCard.jsx`

### 🔍 情绪分析引擎优化
- **修复否定词部分匹配逻辑 bug**：当一个情绪规则中部分关键词被否定、部分未被否定时，旧逻辑只计算未否定词的权重，否定词被忽略。新逻辑分别计算正向和翻转结果，取权重更高者
- **新增部分否定弱化处理**：如"不开心但也不是很难过"（混合情绪），极端得分适度弱化（±1），避免过拟合

### 🏗️ 代码重构
- **AnnualReport 拆分为独立组件**：从 StatsPage.jsx（1156 行）中拆出 `AnnualReport.jsx`（~400 行），StatsPage 从 ~1150 行降至 ~740 行
- 新增 `src/pages/AnnualReport.jsx`

### 🛡️ 安全与监控
- **新增错误监控模块** (`src/services/errorMonitor.js`)
  - 全局 JS 运行时错误捕获
  - 未处理 Promise rejection 捕获
  - 错误队列持久化（localStorage，最多 50 条）
  - 支持手动捕获和导出错误报告
  - 应用启动时自动初始化
- **storage.js 错误处理增强**：`getAllRecords()` 增加 try-catch 错误日志输出，便于调试

### ⚡ Service Worker 优化
- 版本号升级 `moodtrace-v4`
- API 请求策略从"纯网络"改为"网络优先 + 5 秒超时降级"
  - 超时时返回结构化 JSON 错误响应（含中文提示）
  - 群体统计数据在网络波动时不再无限挂起

### 🌍 国际化
- 6 种语言新增"情绪洞察"翻译文案（insight.title）

### 📝 文档更新
- README / CHANGELOG 版本号同步至 v2.1.3
