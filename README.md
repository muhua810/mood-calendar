...
| 🤖 **AI 情绪分析** | 三级降级策略：Workers AI 代理 + 140+ 关键词引擎 + 29 否定词 + Emoji 映射 + 分句加权 + 反讽检测 + 网络用语 |
| 📊 **情绪热力图日历** | 自研 SVG 组件，GitHub 贡献图风格，年/月双视图 |
| 📈 **多维度统计** | 饼图/趋势图/柱状图 + 工作日/周末分析 + 年度报告 |
| 💛 **渐进式关怀** | 3 级关怀系统：轻度鼓励 → 建议放松 + 热线 → 紧急提示，动态升级 |
| 🔒 **隐私优先** | 纯前端 + AES-256-GCM 加密存储，数据不出设备 |
| 📱 **PWA 支持** | 离线缓存、可安装到手机桌面 |
| ☁️ **云端备份** | 基于设备 ID 的无注册备份/恢复，Worker + KV 存储 |
| ♿ **无障碍** | 44px 触摸热区、键盘操作、屏幕阅读器兼容 |

## 🔬 技术创新

### 创新点 1：中文情绪三级分析 + 分句加权 + 反讽检测
...
**网络用语支持**：yyds、emo、破防、蚌埠住了、摆烂、躺平、润了等 20+ 条当代网络用语
...
不再只是简单的"3 天预警"，而是根据连续低落天数和情绪强度**动态升级关怀等级**：
- **Level 1（轻度）**：1-2 天低落 → 温柔鼓励，提供放松建议
- **Level 2（中度）**：3-4 天低落 → 深度关怀，显示心理热线
- **Level 3（重度）**：5+ 天低落 → 紧急提示，心理热线高亮显示
...
│  POST /api/backup/save   — 云端备份上传     │
│  GET  /api/backup/restore — 云端备份恢复    │
│  ┌──────────────────────────────────────┐  │
│  │         Cloudflare KV                │  │
│  └──────────────────────────────────────┘  │
└────────────────────────────────────────────┘
...
- **人文关怀**：3 级渐进式关怀 + 心理热线 + 危机关键词检测，用代码传递温度
